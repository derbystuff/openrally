...
class Server extends LazyEvents{
  ...
  start(callback){
    ...
    this.hapi.on('request-error', this.handleError);
    this.hapi.on('log', this.handleLog);
    this.hapi.connection({host: HOST, port: PORT});
    this.hapi.ext('onRequest', this.handleRequest)
    this.hapi.ext('onPreResponse', this.handlePreRequest);
